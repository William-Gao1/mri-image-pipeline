...
#A simple script to reorient a NIfTI file to RPI (FSL's MNI152, preferred orientation)
#
...
if [ "${native_orient}" != "RPI" ]; then	
  ...
  echo "...Flipping file by ${flipFlag}"

  #Reorienting image and checking for warning messages
  warnFlag=`fslswapdim ${infile} ${flipFlag} ${infile}`
